...
const portfolio = [
	{ assetId: "1", quantity: 5 },
	{ assetId: "2", quantity: 10 },
];
...
// Fetch prices for specified assets and date
...
// 	if (from) {
// 		console.log(new Date(from).toISOString().split("T")[0]);
// 	}
...
// 		// If asOf date is provided, return only the latest prices
...
// Fetch user portfolio for a specific date
router.get("/portfolio", (req, res) => {
	...
	if (!asOf) {
		return res
			.status(400)
			.json({ error: "Missing required parameter: asOf" });
	}

	const asOfDate = new Date(asOf);

	const userPortfolio = portfolio.map((position) => {
	...
	res.send({ date: asOf, positions: userPortfolio });
});
